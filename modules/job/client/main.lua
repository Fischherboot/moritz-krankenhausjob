local GetPlayerServerId            = GetPlayerServerId
    ...
    function createDistressCall()
        if player.distressCallTime then
            ...
            if currentTime - player.distressCallTime < 60000 * timeForNewCall then
                return utils.showNotification(locale("distress_call_in_cooldown"))
            end
        end
        ...
        -- In a hospital job we still call Config.sendDistressCall, however this
        -- callback is intentionally empty in config.lua.  It can be overridden to
        -- integrate with your own notification system if desired.
        ...
        -- Previously this resource would dispatch the distress call to all EMS players:
        -- TriggerServerEvent("ars_ambulancejob:createDistressCall", data)
        -- For the hospital job we do not dispatch calls.  We simply record the time so
        -- players cannot spam the command.
    ...
    -- Only register the command if one is defined.  When Config.helpCommand is false
    -- this section will be skipped, preventing a /911 command for hospital staff.
    if helpCommandName then
